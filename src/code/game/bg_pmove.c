...
static void PM_BeginWeaponChange( int weapon ) {
	...
	pm->ps->weaponTime += pm->fastWeapSwitch ? 20 : 200; // ~Dimmskii
...
static void PM_FinishWeaponChange( void ) {
	...
	pm->ps->weaponTime += pm->fastWeapSwitch ? 25 : 250; // ~Dimmskii
...
//	if ( !(pm->ps->pm_flags & PMF_RESPAWNED) && pm->ps->pm_type != PM_INTERMISSION && pm->ps->pm_type != PM_NOCLIP
	if ( !(pm->ps->pm_flags & PMF_RESPAWNED) && !(pm->ps->pm_flags & PMF_NOSHOOT) && pm->ps->pm_type != PM_INTERMISSION && pm->ps->pm_type != PM_NOCLIP // ~DIMMSKII
