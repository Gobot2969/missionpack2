...
// ~DIMMSKII
//#ifdef MISSIONPACK
	// Remove bounce flag after one time if it's a nailgun bolt ~DIMMSKII
...
//#ifdef MISSIONPACK
	if ( other->takedamage ) {
		...
		// Quickly remove bounce flag if it's a nailgun bolt and the other entity takes damage ~DIMMSKII
		if ( ent->s.weapon != WP_NAILGUN ) {
			ent->s.eFlags &= ~EF_BOUNCE;
		}
	}
